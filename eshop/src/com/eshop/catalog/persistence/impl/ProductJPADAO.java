...
public class ProductJPADAO extends GenericJPADAO<Product, Long> implements ProductDAO {

	private Logger logger = LoggerFactory.getLogger(ProductJPADAO.class);
	...
	public List<Product> findProductsByDimension(List<Dimension> dimensions) {
		...
		Predicate dimensionNameCondition = cb.equal(dimensionJoin.get("dimensionProperty").get("name"),
				cb.parameter(String.class, "dimensionPropertyName0"));
	...
	public Subquery<Product> addDimensionExistsSubQueryRecursively(List<Dimension> dimensions, int index, Subquery<Product> sq,
			CriteriaBuilder cb) {
		...
		Join<ProductSpec, Dimension> dimensionJoin = productSpecJoin.join("dimensions");
		Predicate dimensionNameCondition = cb.equal(dimensionJoin.get("dimensionProperty").get("name"),
				cb.parameter(String.class, "dimensionPropertyName" + index));
		Predicate dimensionValueCondition = cb.equal(dimensionJoin.get("dimensionValue"),
				cb.parameter(String.class, "dimensionValue" + index));
		Predicate dimensionCondition = cb.and(dimensionNameCondition, dimensionValueCondition);
	...
	public Product update(Product detachedInstance) {
		Product persistentInstance = update(detachedInstance, false);
		return persistentInstance;
	}

	@Override
	public Product update(Product detachedInstance, Boolean flush) {
		Product persistentInstance = findForUpdate(detachedInstance.getId(), detachedInstance.getVersion());
